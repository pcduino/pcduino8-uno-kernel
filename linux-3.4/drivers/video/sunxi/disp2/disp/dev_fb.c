...
{
	printk("Fb_check_var: %dx%d %dbits\n", var->xres, var->yres,
	      var->bits_per_pixel);
...
static int sunxi_fb_set_par(struct fb_info *info)
{
	...
	printk("===%s %d ok x=%d, y=%d ==\n", __FUNCTION__, __LINE__, info->var.xres, info->var.yres);
	return 0;
}
...
s32 fb_init(struct platform_device *pdev)
{
	...
	for(i=0; i<2; i++) {
		...
		g_fbi.fbinfo[i]->var.xres            = 1280;
		g_fbi.fbinfo[i]->var.yres            = 720;
		g_fbi.fbinfo[i]->var.xres_virtual    = 1280;
		g_fbi.fbinfo[i]->var.yres_virtual    = 720*2;
		...
		// by liaods, allocate enough memory to support 1080p
		g_fbi.fbinfo[i]->fix.smem_len = (1920*1080*4*2);
