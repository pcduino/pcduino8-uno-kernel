...
int de_rtmx_get_3d_in(unsigned char fmt, de_fb src_size, de_fb *size, de_3d_in_mode trdinmode, unsigned int *addr, unsigned int *trd_addr,
					  unsigned int *pitch, unsigned int *pitchr, unsigned int *lay_laddr)
{
	...
	if(data_type == 0x2) //planar
	{
		if((trdinmode == DE_3D_SRC_MODE_TB))
		{
			...
			lay_laddr[0] = image_w0*crop_h0 + addr[0];
			lay_laddr[1] = image_w1*crop_h1 + addr[1];
			lay_laddr[2] = image_w2*crop_h2 + addr[2];
		}
		...
		else if((trdinmode == DE_3D_SRC_MODE_SSF) || (trdinmode == DE_3D_SRC_MODE_SSH))
		{
			...
			pitchr[1]=pitch[1] = image_w1;
			pitchr[2]=pitch[2] = image_w2;
			lay_laddr[0] = crop_w0 + addr[0];
			lay_laddr[1] = crop_w1 + addr[1];
			lay_laddr[2] = crop_w2 + addr[2];
		}
